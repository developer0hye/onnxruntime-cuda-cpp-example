...
int main(int argc, char *argv[]) { 
  ...
  for (bool use_cuda : {false, true}) {
    for (bool preprocess_on_gpu : use_cuda == false
      ...
      if (use_cuda) {
        if (preprocess_on_gpu)
          diff_gpu_with_cuda_based_opencv_image_preprocessing = end - start;
        else
          diff_gpu = end - start;
      } else
        diff_cpu = end - start;
  ...
  std::cout
      << "Execution time using GPU with cuda based opencv image preprocessing: "
      << diff_gpu_with_cuda_based_opencv_image_preprocessing.count() << " s\n";
