...
public:
  ...
  operator()(const std::vector<cv::Mat> &images) {
    ...
    try {
      ...
      output_shapes[0] = images.size();
      std::vector<float> flatten_batch_output_tensor_values(
          output_tensor,
          output_tensor + calculate_product(output_shapes));
...
int main(int argc, char *argv[]) {
  std::string model_file = "/app/models/mnist_bdynamic.onnx";
  ...
  for (bool use_cuda : {false, true}) {
    ...
    for (int j = 0; j < batch_output.size(); j++) {
      ...
      if (max_index != expected_output_vector[j % expected_output_vector.size()]) {
        test_passed = false;
        std::cerr << "Test failed! For file " << filenames[j % expected_output_vector.size()]
                  << ", expected: " << expected_output_vector[j % expected_output_vector.size()]
