...
int main(int argc, char *argv[]) { 
  ...
  std::map<int, std::pair<double, double>> execution_times;

  for (bool use_trt : std::vector<bool>{false, true}) { 
    OnnxModel model(model_file, use_cuda, use_trt, preprocess_on_gpu);
    model({cv::Mat::zeros(model.get_input_shape()[2], model.get_input_shape()[3], CV_8UC3)}); // warmup

    for (int batch_size : {1, 2, 4, 8, 16, 32, 64})
    {
      ...
      if (count > 0)
      {
        images.erase(images.begin() + count, images.end());
        auto batch_output = model(images);
        images.clear();
      }
      ...
      if (use_trt)
        execution_times[batch_size].second = diff;
      else
        execution_times[batch_size].first = diff;
    }
  }

  std::cout << "Batch Size | Execution Time (ms) without TensorRT | Execution Time (ms) with TensorRT\n";
  for(auto& kv : execution_times) {
